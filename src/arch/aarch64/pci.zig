/// PCI ECAM (Enhanced Configuration Access Mechanism) for AArch64.
///
/// QEMU virt machine ECAM base: 0x4010000000 (above 4 GB).
...
/// On AArch64, no firmware configures PCI BARs, so we do it ourselves.
...
fn assignBars(bus: u8, slot: u8, func: u8) void {
    ...
    for (0..6) |i| {
        ...
        if (bar_val & 0xFFFC != 0) continue;
        var bar_size = probeBarSize(bus, slot, func, bar_offset);
        if (bar_size == 0) bar_size = 0x100;
...
pub fn enumerate() void {
    ...
    for (0..32) |slot_usize| {
        ...
        // Assign I/O BAR addresses before reading them (no firmware does this)
